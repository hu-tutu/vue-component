      ...
      this.mergekeys.forEach(key => {
        ...
        this.listData.forEach((item, index) => {
          if (index === 0) {
            ...
            console.log(this.listData[index][key], this.listData[index - 1][key])
            if (this.listData[index][key] === this.listData[index - 1][key] ||  this.listData[index][key] == '') {
    ...
    objectSpanMethod({ column, rowIndex }) {
      for (let i = 0 ; i < this.mergekeys.length; i++) {
        if (column.property === this.mergekeys[i]) {
          ...
          // console.log(row, columnIndex, _row, '-=-=')
